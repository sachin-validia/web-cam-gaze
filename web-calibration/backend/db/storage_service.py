...
class DatabaseStorageService:
    ...
    def save_calibration_data(self, candidate_id: str, csv_data: str, 
                            transform_matrix: dict) -> bool:
        ...
        try:
            ...
            self.db.execute_query(query, 
                                (candidate_id, csv_data, matrix_bytes, checksum))
            logger.info("Calibration data saved", 
                       candidate_id=candidate_id, checksum=checksum)
            return True
        except Exception as e:
    ...
    def get_calibration_files(self, candidate_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to get calibration files", 
                        candidate_id=candidate_id, error=str(e))
            return None
    ...
    def check_calibration_exists(self, candidate_id: str) -> bool:
        ...
        try:
            ...
            logger.info("Calibration existence check", 
                       candidate_id=candidate_id, exists=exists)
            return exists
            
        except Exception as e:
            logger.error("Failed to check calibration existence", 
                        candidate_id=candidate_id, error=str(e))
            return False
    
    def create_calibration_session(self, session_id: str, candidate_id: str,
                                 interview_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            VALUES (%s, %s, %s, %s)
            """
            self.db.execute_insert(query, (session_id, candidate_id, interview_id, 'in progress'))
            logger.info("Calibration session created", 
                       session_id=session_id, candidate_id=candidate_id)
            return True
        except Exception as e:
            logger.error("Failed to create calibration session", 
    ...
    def save_calibration_frame(self, session_id: str, frame_index: int,
                             frame_data: dict, target_position: dict) -> bool:
        ...
        try:
            ...
            logger.error("Failed to save calibration frame", 
    ...
    def get_calibration_frames(self, session_id: str) -> list:
        ...
        try:
            ...
            logger.error("Failed to get calibration frames", 
                        session_id=session_id, error=str(e))
            return []
    
    def log_audit_event(self, candidate_id: Optional[str], action: str,
        ...
        try:
            ...
            self.db.execute_insert(query, (
                candidate_id, action, user_agent, ip_address, details_json
            ))
            return True
        except Exception as e:
