...
class FileGenerator:
    ...
    def generate_screen_info(self, candidate_id: str, screen_data: dict) -> dict:
        ...
        screen_info = {
            ...
            "screen_width_px": int(screen_data.get("screen_width_px", 1920)),
            "screen_height_px": int(screen_data.get("screen_height_px", 1080)),
            "screen_width_mm": float(screen_data.get("screen_width_mm", 0)),
            "screen_height_mm": float(screen_data.get("screen_height_mm", 0)),
            "camera_position": screen_data.get("camera_position", "top-center"),
            "distance_cm": str(screen_data.get("distance_cm", "60")),
            "dpi": float(screen_data.get("dpi", 96.0)),
            "diagonal_inches": float(screen_data.get("diagonal_inches", 0)),
        ...
        logger.info("Screen info generated", 
                   candidate_id=candidate_id,
    ...
    def generate_calibration_csv(self, calibration_csv_content: str) -> str:
        ...
        logger.info("Calibration CSV validated", 
    ...
    def generate_transform_matrix(self, transform_data: dict) -> bytes:
        ...
        logger.info("Transform matrix NPZ generated", 
                   size_bytes=len(npz_bytes),
                   matrices=list(arrays_to_save.keys()))
        
        return npz_bytes
    
    def save_files(self, candidate_id: str, screen_info: dict, 
                   calibration_csv: str, transform_matrix_bytes: bytes) -> Dict[str, Path]:
        """
        ...
        logger.info("Calibration files saved", 
                   candidate_id=candidate_id,
                   files=list(file_paths.keys()))
        
        return file_paths
    
    def validate_file_compatibility(self, candidate_id: str, 
                                   screen_info: dict,
                                   calibration_csv: str, 
                                   transform_matrix_bytes: bytes) -> Dict[str, Any]:
        """
        ...
        required_screen_fields = [
            'candidate_id', 'timestamp', 'screen_width_px', 'screen_height_px'
        ]
        for field in required_screen_fields:
            if field not in screen_info:
                validation_result['errors'].append(f"Missing screen info field: {field}")
        ...
        try:
            ...
            if 'STransG' not in npz_data:
                validation_result['errors'].append("Missing STransG in transform matrix")
                validation_result['valid'] = False
            else:
        ...
        validation_result['checksums'] = {
            ...
            'calibration_csv': hashlib.sha256(
                calibration_csv.encode()
            ).hexdigest()[:8],
            'transform_matrix': hashlib.sha256(
                transform_matrix_bytes
            ).hexdigest()[:8]
        }
        
        logger.info("File compatibility validated", 
                   candidate_id=candidate_id,
